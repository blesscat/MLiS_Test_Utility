...
class GUI_APP(tk.Tk):
    def __init__(self):
        tk.Tk.__init__(self, className=' MLiS_Test_Utility__201500813')
...
#{{{
        ...
        #    while True:
        #        time.sleep(0.5)
        #        self.serial_port_box['values'] = get_serial_port.serial_ports()
        #t1 = threading.Thread(target=reflash)
        #t1.daemon = True
        #t1.start()
...
#Create a 'textpad' frame and scrollable text, and pack them.
        ...
        self.serial_port_box_value = StringVar()
        self.serial_port_box = ttk.Combobox(self, textvariable=self.serial_port_box_value)
        self.serial_port_box['values'] = get_serial_port.serial_ports()
        ...
        self.serial_port_box.pack(in_=serial_toolbar, side="left")
...
#{{{
        ...
        #print(self.serial_port_box.get())
        #print(type(self.serial_port_box.get()))
        print(builtins.WAIT)
#}}}
...
#{{{
    ...
    def serial_output_open(self):
        #{{{
    #try:
        com_port = self.serial_port_box.get()
        ...
        def callback():
            try:
                while True:
                    ...
                    self.serial_text.configure(state="normal")
                    if self.serial_scrl.get()[1] > 0.99:
    ...
    def serial_output_close(self):
        ...
        self.ser.close()
        self.serial_text.configure(state="normal")
        self.serial_text.insert('end', '{0} has been closed'.format(self.serial_port_box.get()))
        self.serial_text.configure(state="disabled")
        #}}}
            ...
            def callback():
                try:
                    while True:
                        ...
                        self.client_text.configure(state="normal")
                        if self.client_scrl.get()[1] > 0.99:
